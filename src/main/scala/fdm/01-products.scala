...
object tuples {
   ...
   */
  lazy val creditCard: CreditCard = ("1234567812345678", java.time.YearMonth.of(2024, 11), "Andreas Gies", 777)
}
...
object case_class_basics {
  ...
  lazy val creditCard: CreditCard = CreditCard(
    "1234567812345678",
    java.time.YearMonth.of(2024, 11),
    "Andreas Gies",
    777
  )
}
...
object case_class_utilities {
  ...
  val p1 = Person("Andreas Gies", 52)
  val p2 = Person("Karin Gies", 54)
   ...
   */
  lazy val comparison: Boolean = p1 == p2
   ...
   */
  lazy val hashComparison: Boolean = p1.hashCode() == p2.hashCode()
...
object product_patterns {
  ...
  def example1 =
    sherlockHolmes match {
      case Person(name, age) => println(s"$name, age $age")
    }
  ...
  def example2 =
    ("Suitcase", 19.95) match {
      case (name, price) => println(s"$name, price: $price")
    }
   ...
   */
  dilbert match {
    case Employee("Dilbert", _) => println("Hi Dilbert")
    case Employee(name, _)      => println(s"Hello, $name")
  }
   ...
   */
  dilbert match {
    case Employee(_, Address(s, _)) if s.startsWith("B") => println(s"Hello from B street:: $s")
    case Employee(_, Address(s, _))                      => println(s"No B street : $s")
  }
   ...
   */
  dilbert match {
    case Employee(_, a @ Address(_, _)) => println(s"Dilberts address is $a")
  }
   ...
   */
  dilbert match {
    case Employee(n @ ("Dilbert" | "dilbert"), a) => println(s"$n's address is $a")
    case _                                        => // do nothing
  }
}
